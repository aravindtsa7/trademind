...
import { MaterializeCanonicalStorage, RunNifty2024MaterializeStorageOptions, runNifty2024MaterializeStorage } from './research-nifty-2024-materialize-storage';
...
const SPECIAL_ROW_COUNTS: Readonly<Record<string, number>> = { [MAR2_DATE]: 105, [MAY18_DATE]: 105, [MUHURAT_DATE]: 60 };
...
/** 244 ordinary regular dates + the four certified special sessions + the composite-repaired date = 249. */
function allDates(): string[] {
  return [...sequentialDates(244, '2020-01-01'), JAN20_DATE, MAR2_DATE, MAY18_DATE, MUHURAT_DATE, COMPOSITE_REPAIRED_DATE];
}

function fakeDatasetManifest(datasetChecksumOverride?: string): DatasetManifest {
  const identity: UnderlyingSessionIdentity = { datasetKind: ManifestDatasetKind.UNDERLYING_1M, provider: HistoricalProviderId.UPSTOX, instrumentKey: 'NSE_INDEX|Nifty 50', timeframe: '1minute', tradingDate: '2024-01-02' };
  ...
  return {
    ...
    provenance: { provider: HistoricalProviderId.UPSTOX, datasetKind: ManifestDatasetKind.UNDERLYING_1M, instrumentDescriptor: 'NSE_INDEX|Nifty 50', requestedFromDate: '2024-01-01', requestedToDate: '2024-12-31', acquisitionPath: 'PERSISTED_STORE_RECONSTRUCTION', gitRevision: null },
...
function fakeSourceAssembly(overrides: { checksum?: string; sessions?: ResearchSessionSourceSelection[] } = {}): ResearchUnderlyingDatasetAssemblyV1 {
  const sessions = overrides.sessions ?? allDates().map((d) => (d === COMPOSITE_REPAIRED_DATE ? compositeRepairedSelection(d) : realSelection(d)));
  return {
    schemaVersion: 1,
    assemblySemanticsVersion: 1,
    identity: { instrumentKey: 'NSE_INDEX|Nifty 50', timeframe: '1minute', year: 2024 },
...
async function run(materializer: MaterializeCanonicalStorage, checksums: { canonical?: string | undefined; assembly?: string | undefined } = {}): Promise<{ success: boolean; lines: string[]; errorLines: string[] }> {
  ...
  const success = await runNifty2024MaterializeStorage({
    ...
    output,
    errorOutput,
  } as RunNifty2024MaterializeStorageOptions);
...
test('H. any REJECTED_HEALTH_POLICY session present -> FAILED (2024 has no exclusion; zero rejections are expected)', async () => {
  const base = fullyValidResult();
  const materializer = new FakeMaterializer({ ...base, exportResult: { ...base.exportResult, sessions: exportSessionsForAll({}, { [MUHURAT_DATE]: ParquetSessionExportStatus.REJECTED_HEALTH_POLICY }) } });
  const { success, errorLines } = await run(materializer);
  assert.equal(success, false);
...
test('J. Muhurat (2024-11-01) exported rowCount is not 60 -> FAILED', async () => {
  const base = fullyValidResult();
  const materializer = new FakeMaterializer({ ...base, exportResult: { ...base.exportResult, sessions: exportSessionsForAll({ [MUHURAT_DATE]: 61 }) } });
  const { success, errorLines } = await run(materializer);
  assert.equal(success, false);
  assert.ok(errorLines.join('\n').includes('code=SESSION_WRONG_ROW_COUNT'));
});

test('K. a DR-switchover session (2024-03-02) exported rowCount is not 105 -> FAILED', async () => {
  const base = fullyValidResult();
  const materializer = new FakeMaterializer({ ...base, exportResult: { ...base.exportResult, sessions: exportSessionsForAll({ [MAR2_DATE]: 104 }) } });
  const { success, errorLines } = await run(materializer);
  assert.equal(success, false);
  assert.ok(errorLines.join('\n').includes('code=SESSION_WRONG_ROW_COUNT'));
});

test('L. a regular session exported rowCount is not 375 -> FAILED', async () => {
  const base = fullyValidResult();
...
test('M. the composite-repaired session (2024-12-12) exported rowCount is not 375 -> FAILED', async () => {
  const base = fullyValidResult();
  const materializer = new FakeMaterializer({ ...base, exportResult: { ...base.exportResult, sessions: exportSessionsForAll({ [COMPOSITE_REPAIRED_DATE]: 374 }) } });
  const { success, errorLines } = await run(materializer);
  assert.equal(success, false);
  assert.ok(errorLines.join('\n').includes('code=SESSION_WRONG_ROW_COUNT'));
});
...
test('N1. zero composite-repaired sessions in the trusted B-M7.2 selections (2024-12-12 downgraded to real-canonical) -> FAILED', async () => {
  const base = fullyValidResult();
  const materializer = new FakeMaterializer({ ...base, sourceAssembly: fakeSourceAssembly({ sessions: allDates().map((d) => realSelection(d)) }) });
  const { success, errorLines } = await run(materializer);
  assert.equal(success, false);
  assert.ok(errorLines.join('\n').includes('code=WRONG_COMPOSITE_REPAIRED_TRADING_DATES'));
});

test('N2. the composite-repaired tier is present but at the wrong date -> FAILED', async () => {
  const base = fullyValidResult();
  const sessions = allDates().map((d) => (d === '2020-01-02' ? compositeRepairedSelection(d) : d === COMPOSITE_REPAIRED_DATE ? realSelection(d) : realSelection(d)));
...
test('structural: this CLI reads exactly two environment variables (the year-specific checksum handoff) and never invents a placeholder checksum constant', () => {
  const source = readFileSync(join(__dirname, 'research-nifty-2024-materialize-storage.ts'), 'utf8');
