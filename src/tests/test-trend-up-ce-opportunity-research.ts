...
type RsiFilter = 'NO_RSI_FILTER' | 'RSI_GT_50' | 'RSI_GT_55' | 'RSI_GT_60' | 'RSI_GT_65';
...
const rsiFilters: readonly RsiFilter[] = ['NO_RSI_FILTER', 'RSI_GT_50', 'RSI_GT_55', 'RSI_GT_60', 'RSI_GT_65'];
...
function rsiThreshold(filter: RsiFilter): number | undefined {
  ...
  return Number(filter.replace('RSI_GT_', ''));
}
...
function evaluateOpportunity(session: Session, index: number, setup: Setup): Opportunity {
  ...
  horizons.forEach((horizon) => {
    ...
    if (future) movements[horizon] = future.close - signal.close;
  });
  ...
  return {
    ...
    close: signal.close,
    movements,
    mfe: hasFullThirtyMinutes ? Math.max(0, Math.max(...futureThirty.map((candle) => candle.high)) - signal.close) : undefined,
    mae: hasFullThirtyMinutes ? Math.max(0, signal.close - Math.min(...futureThirty.map((candle) => candle.low))) : undefined,
  };
}

function isPullbackOpportunity(session: Session, index: number, config: Config, ema15: number, ema35: number): boolean {
  if (!config.proximity || !pullbackSetups.includes(config.setup)) return false;
  const candle = session.candles[index];
  const anchor = config.setup === 'PULLBACK_EMA15' ? ema15 : ema35;
  const distancePercent = Math.abs(candle.low - anchor) / anchor * 100;
...
function isContinuationOpportunity(session: Session, index: number, config: Config): boolean {
  ...
  const recentHigh = Math.max(...session.candles.slice(index - lookback, index).map((candle) => candle.high));
  return session.candles[index].close > recentHigh;
}
...
async function run(): Promise<void> {
  ...
  logger.info('Starting TREND_UP CE opportunity research', { instrumentKey, sourceTimeframe });
  ...
  logger.info('TREND_UP CE opportunity research completed', { completeSessions: sessions.length, configurations: reports.length });
}

run().catch((error) => {
  logger.error('TREND_UP CE opportunity research failed', { error });
  console.error('TREND_UP CE opportunity research failed.', error);
  process.exitCode = 1;
});
