...
async function run(): Promise<void> {
  ...
  const onTick = (event: unknown) => { const tick = event as Partial<MarketTickEvent>; if (typeof tick.instrumentKey !== 'string' || typeof tick.ltp !== 'number' || typeof tick.timestamp !== 'string') return; const at = new Date(tick.timestamp); if (Number.isNaN(at.getTime())) return; if (isAtOrAfterIstSessionEnd(at)) { eodStarted = true; requestEod?.(); return; } if (eodStarted) return; flush(tracker.observePremium(tick.instrumentKey, tick.ltp, at)); };
  ...
  connection.on('unexpectedDisconnect', (details: { code?: number; reason?: string }) => forwardJournal.appendEvent(forwardDate, 'WEBSOCKET_DISCONNECTED', ['WEBSOCKET_DISCONNECTED'], { code: details.code ?? null, reason: details.reason ?? null }));
  connection.on('reconnected', (details: { downtimeMs?: number }) => forwardJournal.appendEvent(forwardDate, 'WEBSOCKET_RECONNECTED', ['WEBSOCKET_RECONNECTED'], { downtimeMs: details.downtimeMs ?? null }));
