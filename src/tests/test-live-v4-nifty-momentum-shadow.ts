...
async function run(): Promise<void> {
  ...
  let completed3m = 0; let opportunities = 0; let signals = 0; let closing = false; let eodStarted = false; let requestEod: (() => void) | undefined; let eodTimer: NodeJS.Timeout | undefined; let eodWatchdog: NodeJS.Timeout | undefined;
  let recoveryWarmup: Awaited<ReturnType<LivePaperFreshWarmupService['warmUp']>> | undefined;
  const recovery = new MarketDataRecoveryCoordinatorService({
    backfill: async () => {
      recoveryWarmup = await new LivePaperFreshWarmupService(new HistoricalCandleRepository(), new ShadowWarmupTarget()).warmUp();
      return { ready: recoveryWarmup.ready, reason: recoveryWarmup.freshnessReason, missingMinutes: recoveryWarmup.currentDayMissingMinuteCount, duplicateMinutes: recoveryWarmup.currentDayDuplicateCount };
    },
    onRecovered: () => { if (recoveryWarmup) { evaluator.recoverHistoricalOneMinute(recoveryWarmup.seededOneMinuteCandles); candleEvents.start(); } },
  ...
  const onTick = (event: unknown) => { const tick = event as Partial<MarketTickEvent>; if (typeof tick.instrumentKey !== 'string' || typeof tick.ltp !== 'number' || typeof tick.timestamp !== 'string') return; const at = new Date(tick.timestamp); if (Number.isNaN(at.getTime())) return; if (isAtOrAfterIstSessionEnd(at)) { eodStarted = true; requestEod?.(); return; } if (eodStarted) return; if (tick.instrumentKey === nifty) recovery.handleLiveTick(at); if (!recovery.isEvaluationReady()) return; flush(tracker.observePremium(tick.instrumentKey, tick.ltp, at)); };
  ...
  const handleCandle = async (event: unknown): Promise<void> => {
    ...
    if (eodStarted || !recovery.isEvaluationReady() || candle.instrumentKey !== nifty || candle.completed !== true || !(candle.candleTime instanceof Date) || ![candle.open,candle.high,candle.low,candle.close].every((value) => typeof value === 'number' && Number.isFinite(value))) return;
  ...
  connection.on('unexpectedDisconnect', (details: { code?: number; reason?: string }) => { recovery.handleUnexpectedDisconnect(); candleEvents.stop(); forwardJournal.appendEvent(forwardDate, 'WEBSOCKET_DISCONNECTED', ['WEBSOCKET_DISCONNECTED'], { code: details.code ?? null, reason: details.reason ?? null }); });
  connection.on('reconnected', (details: { downtimeMs?: number }) => { forwardJournal.appendEvent(forwardDate, 'WEBSOCKET_RECONNECTED', ['WEBSOCKET_RECONNECTED'], { downtimeMs: details.downtimeMs ?? null }); recovery.handleReconnected(); });
  connection.on('reconnectFailed', (details: { attempts?: number; downtimeMs?: number }) => { forwardJournal.appendEvent(forwardDate, 'RECONNECT_FAILED', ['RECONNECT_FAILED', 'DATA_GAP', 'CRITICAL_DATA_QUALITY'], { attempts: details.attempts ?? null, downtimeMs: details.downtimeMs ?? null }); shutdown('RECONNECT_FAILED'); });
  requestEod = () => { void finishEod(); }; eodTimer = eodCoordinator.schedule(finishEod); eodWatchdog = setInterval(() => { if (isAtOrAfterIstSessionEnd(new Date())) void finishEod(); }, 1_000); eodWatchdog.unref();
  process.once('SIGINT', () => { if (eodTimer) clearTimeout(eodTimer); shutdown(); }); process.once('SIGTERM', () => { if (eodTimer) clearTimeout(eodTimer); shutdown(); });
