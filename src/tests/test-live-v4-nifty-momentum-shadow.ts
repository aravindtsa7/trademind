...
async function run(): Promise<void> {
  ...
  const health = new MarketDataHealthMonitorService(connection, { generationGraceMs:healthGraceMs, onStall: (snapshot) => {
  ...
  const finishEod = async (reason = 'VALID_COMPLETED'): Promise<void> => {
    await eodCoordinator.runOnce(new Date(), async () => {
      ...
      // substitute for a missed real-time evaluation opportunity.
      const finalGenerationId = connection.getGenerationId();
      sourceBoundaryEvaluationCoverage.require(finalGenerationId, nifty1mSourceCompletionBoundary(new Date()));
