...
const configurations: readonly AdxFilterConfiguration[] = [
  { id: 'EMA15/35 + RSI55/45 baseline' },
  { id: 'ADX >= 15', adxThreshold: 15 },
  { id: 'ADX >= 20', adxThreshold: 20 },
  { id: 'ADX >= 25', adxThreshold: 25 },
  { id: 'ADX >= 30', adxThreshold: 30 },
];
...
interface AdxFilterConfiguration {
  id: string;
  adxThreshold?: number;
}
...
function getAdxValue(indicatorResults: IndicatorEngineResult, timestamp: Date): number | undefined {
  const matchingResults = indicatorResults.indicators.filter(
    (entry) =>
      entry.config.type === IndicatorType.ADX &&
      'period' in entry.config &&
      entry.config.period === 14
  );
  if (matchingResults.length !== 1) {
    return undefined;
  }
  ...
  return entry && 'adx' in entry && typeof entry.adx === 'number' ? entry.adx : undefined;
}
...
class EmaRsiWithOptionalAdxFilter implements Strategy<ResearchStrategyContext> {
  readonly id: string;

  private readonly emaCross = new EmaCrossStrategy({ fastPeriod: 15, slowPeriod: 35 });

  constructor(private readonly configuration: AdxFilterConfiguration) {
  ...
  evaluate(context: ResearchStrategyContext): StrategySignalDto {
    ...
    const adx = getAdxValue(context.indicatorResults, context.candle.timestamp);
    ...
    if (
      ...
      return noTrade('Required EMA, RSI, ADX, or regime values are not yet aligned at this candle.');
    }
    ...
    if (this.configuration.adxThreshold === undefined) {
      return {
        ...emaCrossSignal,
    ...
    return adx >= this.configuration.adxThreshold
      ? {
          ...emaCrossSignal,
          reasons: [
            ...emaCrossSignal.reasons,
            `RSI 14 ${rsi.toFixed(2)} and ADX 14 ${adx.toFixed(2)} >= ${this.configuration.adxThreshold} confirmed the crossover.`,
          ],
        }
      : noTrade(
          `${emaCrossSignal.signal} EMA15/35 + RSI55/45 signal rejected: ADX 14 ${adx.toFixed(2)} is below ${this.configuration.adxThreshold}.`
        );
  }
}
...
function getRollingWindows(sessions: readonly CompleteSession[]): CompleteSession[][] {
  if (sessions.length < researchWindowSessions) {
    throw new Error(
      `ADX filter stability research requires at least ${researchWindowSessions} complete sessions; found ${sessions.length}.`
    );
  }
...
function printWindow(
  ...
  results: Array<{ configuration: AdxFilterConfiguration; run: ResearchRunResult }>
): void {
  console.log(
...
function printStabilitySummary(stabilityByConfiguration: Map<string, StabilityMetrics>): void {
  ...
  const rankedAdxConfigurations = entries
  ...
  console.log(
    `Most consistently competitive ADX configuration: ${rankedAdxConfigurations[0][0]}. This is not a production promotion or option P&L conclusion.`
  );
}
...
async function run(): Promise<void> {
  ...
  logger.info('Starting ADX filter stability research integration test', {
    instrumentKey,
  ...
  windows.forEach((sessions, index) => {
    ...
    const results = configurations.map((configuration) => ({
      ...
      run: runner.run(
        candles,
        createConfig(new EmaRsiWithOptionalAdxFilter(configuration), fromDate, toDate)
      ),
    }));
  ...
  logger.info('ADX filter stability research integration test completed', {
    instrumentKey,
...
run().catch((error) => {
  logger.error('ADX filter stability research integration test failed', { error });
  console.error('ADX filter stability research integration test failed.', error);
  process.exitCode = 1;
});
