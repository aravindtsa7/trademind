...
async function run(): Promise<void> {
  ...
  const { sourceDatasetKind, identity, contentChecksum, sourceRows, canonicalRowCount } =
    datasetKindRaw === ManifestDatasetKind.UNDERLYING_1M
      ? await loadUnderlyingSession(sessionBuilder, tradingDate, start, end)
      : await loadOptionSession(sessionBuilder, tradingDate, start, end);

  console.log(JSON.stringify({ event: 'research:resample starting', datasetKind: datasetKindRaw, tradingDate, targetTimeframe, sourceRowCount: canonicalRowCount }));
...
async function loadUnderlyingSession(sessionBuilder: DatasetSessionManifestBuilderService, tradingDate: string, start: Date, end: Date) {
  ...
  const manifest = sessionBuilder.buildUnderlyingSession({ provider, instrumentKey, timeframe, tradingDate, rows });
...
async function loadOptionSession(sessionBuilder: DatasetSessionManifestBuilderService, tradingDate: string, start: Date, end: Date) {
