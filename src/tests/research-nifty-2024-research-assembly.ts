...
/**
 * B-M10 operator runner: assembles the deterministic NIFTY underlying
 * research-layer overlay for locked year 2024. UNLIKE 2023 (a CLEAN
 * canonical year), 2024 carries exactly ONE known composite-repaired
 * canonical session -- 2024-12-12, accepted through the existing
 * `NiftyUnderlyingGapRepairService` composite-repair path after a genuine
 * primary-provider gap (Upstox 374/375, missing 09:42 IST) was filled from a
 * secondary provider and persisted as a single canonical 375-row session.
 * This CLI does NOT hardcode an assumption that all 249 sessions are pure
 * primary -- it asserts the exact expected per-tier topology
 * (248 real-canonical / 1 composite-repaired / 0 authorized-derived / 0
 * unavailable) and independently pins the ONE composite-repaired trading
 * date to `2024-12-12` specifically, failing closed if any OTHER date
 * unexpectedly appears in that tier instead. Reuses
 * `NiftyUnderlyingResearchAssemblyService` unmodified -- no second
 * source-selection algorithm; the actual tier classification for
 * 2024-12-12 comes entirely from the existing, already-reviewed
 ...
 * 2022/2023 BLOCKER-04 discipline.
 *
 * TERRA BLOCKER CORRECTION (carried forward from 2023): per-tier COUNT
 ...
 * re-derives the exact certified 2024 trading-date set from
 ...
 * BEFORE persistence, plus exact window validation for all FOUR certified
 * 2024 special sessions (2024-01-20, 2024-03-02, 2024-05-18, 2024-11-01
 * Muhurat).
 *
 * Usage (PowerShell), once reviewed:
 *   npm run research:nifty-2024:assemble
 */
...
/** The ONE controlled, reviewed 2024 composite repair (task-locked fact): Upstox primary=374/375 (missing 09:42 IST), repaired from a secondary provider to a real 375-row canonical session. */
const EXPECTED_COMPOSITE_REPAIRED_TRADING_DATES: readonly string[] = ['2024-12-12'];

/** Certified 2024 special sessions (task-locked facts, cross-checked against the authoritative NSE/EQUITY fixture). */
...
/** Both 2024-03-02 and 2024-05-18 share the identical two-window DR-switchover shape: 09:15-10:00 (45m) + 11:30-12:30 (60m), never bridging the 10:00-11:30 gap. */
...
/** All FOUR certified 2024 special sessions -- every one must be present in the assembly AND independently resolve to exactly these certified calendar windows. */
const LOCKED_SPECIAL_SESSIONS: readonly LockedSpecialSession[] = [
  { date: JAN20_DATE, windows: JAN20_WINDOWS, missingCode: 'MISSING_JAN20_SPECIAL_SESSION', windowMismatchCode: 'JAN20_WINDOW_MISMATCH' },
  { date: MAR2_DATE, windows: DR_SWITCHOVER_WINDOWS, missingCode: 'MISSING_MAR2_SPECIAL_SESSION', windowMismatchCode: 'MAR2_WINDOW_MISMATCH' },
  { date: MAY18_DATE, windows: DR_SWITCHOVER_WINDOWS, missingCode: 'MISSING_MAY18_SPECIAL_SESSION', windowMismatchCode: 'MAY18_WINDOW_MISMATCH' },
  { date: MUHURAT_DATE, windows: MUHURAT_WINDOWS, missingCode: 'MISSING_MUHURAT_SESSION', windowMismatchCode: 'MUHURAT_WINDOW_MISMATCH' },
];

/** The exact service surface this CLI needs -- `assembleYear` (never persists on its own here) plus the separate `persistAssembly` step, mirroring the 2022/2023 runners. */
...
export interface RunNifty2024ResearchAssemblyOptions {
 ...
 * research assembly. Deliberately hardcodes 249/248/1/0/0 -- acceptable and
 ...
 * require zero composite-repaired sessions -- it requires EXACTLY the one
 * controlled, reviewed 2024-12-12 repair, and fails closed if any OTHER date
...
function validateLockedProductionPostconditions(assembly: ResearchUnderlyingDatasetAssemblyV1): PostconditionFailure | null {
  ...
  if (sessionCounts.authorizedDerivedSessions !== EXPECTED_AUTHORIZED_DERIVED_SESSIONS) {
    return { code: 'UNEXPECTED_AUTHORIZED_DERIVED_SESSIONS', message: `expected 0 authorized-derived sessions for 2024, got ${sessionCounts.authorizedDerivedSessions}` };
  }
  if (sessionCounts.compositeRepairedSessions !== EXPECTED_COMPOSITE_REPAIRED_SESSIONS) {
    return { code: 'WRONG_COMPOSITE_REPAIRED_SESSION_COUNT', message: `expected exactly ${EXPECTED_COMPOSITE_REPAIRED_SESSIONS} composite-repaired session for 2024 (the reviewed 2024-12-12 repair), got ${sessionCounts.compositeRepairedSessions}` };
  }
  ...
  if (!arraysEqual(compositeRepairedTradingDates, EXPECTED_COMPOSITE_REPAIRED_TRADING_DATES)) {
    return {
      code: 'WRONG_COMPOSITE_REPAIRED_TRADING_DATES',
      message: `expected the composite-repaired trading date set to be exactly [${EXPECTED_COMPOSITE_REPAIRED_TRADING_DATES.join(',')}], got [${compositeRepairedTradingDates.join(',')}]`,
    };
  }
  ...
  if (nonQualifyingDates.length > 0) {
    return { code: 'NON_REAL_CANONICAL_OR_COMPOSITE_REPAIRED_SESSION_PRESENT', message: `expected every selected 2024 date to be real-canonical or the one reviewed composite-repaired date, found unexpected-tier date(s): [${nonQualifyingDates.join(',')}]` };
  }
...
/**
 * TERRA BLOCKER CORRECTION (carried forward from 2023): requires EXACT set
 * equality between the assembly's own trading dates and the certified 2024
 * trading-date set independently resolved from `certified`, plus exact
 * window validation for all FOUR certified 2024 special sessions.
 */
function validateCertifiedDateSetPostcondition(assembly: ResearchUnderlyingDatasetAssemblyV1, certified: ManifestRequestedSessions): PostconditionFailure | null {
  ...
  if (certifiedDates.length !== EXPECTED_SESSIONS) {
    return {
      ...
      message: `the certified ${LOCKED_YEAR} calendar itself resolved ${certifiedDates.length} trading date(s), expected exactly ${EXPECTED_SESSIONS} -- refusing to validate the assembly against a calendar source that disagrees with the locked 2024 topology`,
    };
  }
 ...
 */
export async function runNifty2024ResearchAssembly(options: RunNifty2024ResearchAssemblyOptions): Promise<boolean> {
  ...
  try {
    ...
    errorOutput(['[B_M10_2024_RESEARCH_ASSEMBLY]', 'status=FAILED', 'code=ASSEMBLY_FAILED', `name=${name}`, `message=${message}`].join('\n'));
    return false;
  }

  const postconditionFailure = validateLockedProductionPostconditions(result.assembly);
  if (postconditionFailure) {
    errorOutput(['[B_M10_2024_RESEARCH_ASSEMBLY]', 'status=FAILED', `code=${postconditionFailure.code}`, `message=${postconditionFailure.message}`].join('\n'));
    return false;
  }
  ...
  try {
    ...
    const name = error instanceof Error ? error.name : 'UnknownError';
    const message = error instanceof Error ? error.message : String(error);
    errorOutput(['[B_M10_2024_RESEARCH_ASSEMBLY]', 'status=FAILED', 'code=CERTIFIED_CALENDAR_RESOLUTION_FAILED', `name=${name}`, `message=${message}`].join('\n'));
    return false;
  }

  const dateSetFailure = validateCertifiedDateSetPostcondition(result.assembly, certified);
  if (dateSetFailure) {
    errorOutput(['[B_M10_2024_RESEARCH_ASSEMBLY]', 'status=FAILED', `code=${dateSetFailure.code}`, `message=${dateSetFailure.message}`].join('\n'));
    return false;
  }
  ...
  try {
    ...
    const name = error instanceof Error ? error.name : 'UnknownError';
    const message = error instanceof Error ? error.message : String(error);
    errorOutput(['[B_M10_2024_RESEARCH_ASSEMBLY]', 'status=FAILED', 'code=PERSISTENCE_FAILED', `name=${name}`, `message=${message}`].join('\n'));
    return false;
  }
...
async function main(): Promise<void> {
  const success = await runNifty2024ResearchAssembly({
...
if (require.main === module) {
  main().catch((error) => {
    console.error('[B_M10_2024_RESEARCH_ASSEMBLY] status=FAILED code=RUNNER_CRASH', error instanceof Error ? error.message : String(error));
    process.exitCode = 1;
  });
}
