...
/**
 * B-M10 operator runner: reconstructs the exact certified 2024 NIFTY
 ...
 * verifies canonical Parquet for all 249 sessions -- unlike 2022, there is NO
 * authorized-derived canonical-empty date in 2024, so no session is ever
 * excluded by B-F6's health policy. The one composite-repaired session
 * (2024-12-12) is preflighted/exported/verified through the EXACT SAME
 * tier 1/2 path as every real-canonical session (see
 * `NiftyUnderlyingCanonicalStorageMaterializerService.preflight`'s existing,
 * unmodified `HEALTHY_REAL_CANONICAL_SESSION`/`ACCEPTED_COMPOSITE_REPAIRED_CANONICAL_SESSION`
 * case) -- no special Parquet format, no separate export path. Reuses
 * `NiftyUnderlyingCanonicalStorageMaterializerService` unmodified -- no new
 * Parquet exporter, no new verifier.
 *
 * CHECKSUM HANDOFF: the 2024 canonical dataset checksum and research assembly
 * checksum are not known until `research:nifty-2024:assemble` has actually
 * been run for real, so this CLI requires BOTH as explicit operator inputs
 * via the narrow, year-specific `RESEARCH_NIFTY_2024_CANONICAL_MANIFEST_CHECKSUM`
 * and `RESEARCH_NIFTY_2024_SOURCE_ASSEMBLY_CHECKSUM` environment variables
 * (the latter is the SAME value the operator supplied to
 * `research:nifty-2024:resample`). Both values are format-validated (exactly
 ...
 *   $env:RESEARCH_NIFTY_2024_CANONICAL_MANIFEST_CHECKSUM = "<canonicalManifestChecksum>"
 *   $env:RESEARCH_NIFTY_2024_SOURCE_ASSEMBLY_CHECKSUM = "<researchAssemblyChecksum>"
 *   npm run research:nifty-2024:materialize-storage
 */

const LOCKED_YEAR = 2024;
const EXPECTED_SESSIONS = 249;
const REGULAR_ROW_COUNT = 375;
/** The three certified 2024 sessions whose exported rowCount differs from the ordinary 375-row day -- every other exported session (including 2024-01-20 and the 2024-12-12 composite repair, both ordinary 375-row full days) defaults to `REGULAR_ROW_COUNT`. */
...
export interface RunNifty2024MaterializeStorageOptions {
  /** Operator-supplied upstream checksums (normally read from the two RESEARCH_NIFTY_2024_* environment variables) -- injected explicitly here so tests never read real environment variables. */
...
function validateChecksumInputs(canonicalManifestChecksum: string | undefined, sourceAssemblyChecksum: string | undefined): ValidatedChecksumInputs | PostconditionFailure {
  if (!canonicalManifestChecksum) {
    return { code: 'MISSING_CANONICAL_MANIFEST_CHECKSUM', message: `environment variable ${CANONICAL_MANIFEST_CHECKSUM_ENV_VAR} must be set to the exact 2024 canonicalManifestChecksum produced by 'npm run research:nifty-2024:assemble'` };
  }
  ...
  if (!sourceAssemblyChecksum) {
    return { code: 'MISSING_SOURCE_ASSEMBLY_CHECKSUM', message: `environment variable ${SOURCE_ASSEMBLY_CHECKSUM_ENV_VAR} must be set to the exact 2024 researchAssemblyChecksum produced by 'npm run research:nifty-2024:assemble'` };
  }
...
/**
 * The ONE locked production postcondition gate for the 2024 canonical
 * storage materialization. Deliberately hardcodes 249/105/105/60/375 --
 ...
 * Like 2023, asserts ZERO rejected-health-policy sessions (2024 has no
 * authorized-derived canonical-empty date to exclude). UNLIKE 2023, does NOT
 * independently re-derive the composite-repaired tier from the exported
 * Parquet sessions (row-count/status alone cannot distinguish tier 1 from
 * tier 2) -- it instead cross-checks the tier classification directly from
...
function validateLockedPostconditions(result: MaterializeCanonicalStorageResult, expectedChecksums: ValidatedChecksumInputs): PostconditionFailure | null {
  ...
  if (rejectedHealthPolicy.length !== 0) {
    return { code: 'UNEXPECTED_REJECTED_HEALTH_POLICY_SESSION', message: `expected zero REJECTED_HEALTH_POLICY sessions for 2024, got [${rejectedHealthPolicy.map((s) => s.tradingDate).join(',')}]` };
  }
  ...
  // Cross-checks the ONE reviewed composite-repaired date directly against the trusted B-M7.2
  ...
  const compositeRepairedDates = result.sourceAssembly.sessions
    ...
    .sort();
  if (compositeRepairedDates.length !== 1 || compositeRepairedDates[0] !== COMPOSITE_REPAIRED_DATE) {
    return { code: 'WRONG_COMPOSITE_REPAIRED_TRADING_DATES', message: `expected the composite-repaired trading date set to be exactly [${COMPOSITE_REPAIRED_DATE}], got [${compositeRepairedDates.join(',')}]` };
  }
  ...
  if (nonQualifyingDates.length > 0) {
    return { code: 'NON_REAL_CANONICAL_OR_COMPOSITE_REPAIRED_SESSION_PRESENT', message: `expected every 2024 B-M7.2 selection to be real-canonical or the one reviewed composite-repaired date, found unexpected-tier date(s): [${nonQualifyingDates.join(',')}]` };
  }
 ...
 * preflighted, persisted, exported, and verified 2024 canonical storage
 * state; `false` on any failure. Never throws.
 */
export async function runNifty2024MaterializeStorage(options: RunNifty2024MaterializeStorageOptions): Promise<boolean> {
  ...
  if ('code' in checksumInputs) {
    errorOutput(['[B_M10_2024_CANONICAL_STORAGE_MATERIALIZATION]', 'status=FAILED', `code=${checksumInputs.code}`, `message=${checksumInputs.message}`].join('\n'));
    return false;
  }
  ...
  try {
    ...
    errorOutput(['[B_M10_2024_CANONICAL_STORAGE_MATERIALIZATION]', 'status=FAILED', 'code=MATERIALIZE_FAILED', `name=${name}`, `message=${message}`].join('\n'));
    return false;
  }

  const postconditionFailure = validateLockedPostconditions(result, checksumInputs);
  if (postconditionFailure) {
    errorOutput(['[B_M10_2024_CANONICAL_STORAGE_MATERIALIZATION]', 'status=FAILED', `code=${postconditionFailure.code}`, `message=${postconditionFailure.message}`].join('\n'));
    return false;
  }
...
function formatSuccessOutput(result: MaterializeCanonicalStorageResult): string {
  ...
  const compositeRepairedTradingDates = result.sourceAssembly.sessions
    ...
    .join(',');
  return [
    '[B_M10_2024_CANONICAL_STORAGE_MATERIALIZATION]',
    'status=SUCCESS',
    `year=${LOCKED_YEAR}`,
...
async function main(): Promise<void> {
  const success = await runNifty2024MaterializeStorage({
...
if (require.main === module) {
  main().catch((error) => {
    console.error('[B_M10_2024_CANONICAL_STORAGE_MATERIALIZATION] status=FAILED code=RUNNER_CRASH', error instanceof Error ? error.message : String(error));
    process.exitCode = 1;
  });
}
