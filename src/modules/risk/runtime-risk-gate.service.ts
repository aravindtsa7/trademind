...
export default class RuntimeRiskGateService {
  ...
  evaluate(intent: RuntimeRiskIntent): RiskDecision {
    ...
    const currentPositions = positions ?? []; const projectedNotional = intent.entryPremium * intent.quantity; const totalNotional = portfolio?.totalNotional ?? currentPositions.reduce((sum, p) => sum + p.notional, 0);
    const strategySnapshot = portfolio?.strategyBreakdown.find((value) => value.strategyId === intent.strategyId); const underlyingSnapshot = portfolio?.underlyingBreakdown.find((value) => value.underlying === intent.underlying);
    const strategyNotional = strategySnapshot?.totalNotional ?? currentPositions.filter((p) => p.strategyId === intent.strategyId).reduce((sum,p)=>sum+p.notional,0);
    const openCount = portfolio?.openPositionCount ?? currentPositions.length; const strategyOpenCount = strategySnapshot?.openPositionCount ?? currentPositions.filter((p)=>p.strategyId===intent.strategyId).length; const underlyingOpenCount = underlyingSnapshot?.openPositionCount ?? currentPositions.filter((p)=>p.underlying===intent.underlying).length;
    if (intent.action === 'OPEN' && openCount >= this.options.maxOpenPositions) reasons.push('MAX_OPEN_POSITIONS');
    if (intent.action === 'OPEN' && strategyOpenCount >= this.options.maxOpenPositionsPerStrategy) reasons.push('MAX_OPEN_POSITIONS');
    if (intent.action === 'OPEN' && underlyingOpenCount >= this.options.maxOpenPositionsPerUnderlying) reasons.push('MAX_OPEN_POSITIONS');
