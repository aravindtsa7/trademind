...
export default class MarketDataRecoveryCoordinatorService extends EventEmitter {
  ...
  stop(): void { if (this.stopping || this.state === 'STOPPED') return; this.stopping = true; this.setState('STOPPING'); this.setState('STOPPED'); }
  ...
  private async recover(): Promise<void> {
    if (this.stopping) return; this.setState('BACKFILLING'); this.emitEvent('MARKET_DATA_BACKFILL_STARTED', { generationId: this.activeGenerationId });
    try {
      ...
      this.backfillReady = true; this.emitEvent('MARKET_DATA_BACKFILL_COMPLETED', { generationId:this.activeGenerationId, missingMinutes:result.missingMinutes });
  ...
  private tryBecomeReady(): void {
    if ((this.state === 'WAITING_FOR_FRESH_TICK' || this.state === 'AWAITING_LIVE_TICK') && this.backfillReady && this.freshLiveTick) {
      this.setState('READY'); this.emitEvent('DATA_GAP_RECOVERED', { generationId:this.activeGenerationId, durationMs:Date.now()-this.recoveryStartedAt }); this.emitEvent('MARKET_DATA_READY', { generationId:this.activeGenerationId });
    }
  }
